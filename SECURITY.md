...
- No external model or API receives patient-like data.
...
| Sensitive Information Disclosure | Relevant because inputs resemble PHI/PII | No runtime API calls; generated eval/report files are ignored; README says no API key required; outputs avoid copying unrelated patient identifiers. |
...
| Unbounded Consumption | Low current applicability | Local linear scan over one submission; no model token costs or recursive tool use. |
...
| Injection | Relevant for untrusted JSON/text | No SQL, shell, template execution, or `eval`; document text is parsed with deterministic string/date checks only. |
| Insecure Design | Relevant | Conservative state machine: missing/unknown data → `NEEDS_FOLLOW_UP`; acute exclusions → `NOT_CLEARED`. |
| Security Misconfiguration | Low locally | `.gitignore` excludes generated reports and local env files. If deployed, disable debug logs containing PHI. |
